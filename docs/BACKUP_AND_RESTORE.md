...
`deploy/scripts/backup.sh` requires `pg_dump`, `psql`, `gzip`, Python and either `SUPABASE_DB_URL_FILE` (preferred) or `SUPABASE_DB_URL`. The login in that DSN must be allowed to `SET ROLE service_role`; `pg_dump` does so explicitly because inherited membership alone does not inherit the role's `BYPASSRLS` attribute. The script keeps the URL out of process arguments/output, writes with umask `077`, creates `pokecrack-YYYYMMDDTHHMMSSZ.sql.gz`, validates compressed and non-empty uncompressed content, moves atomically, then writes `.last-successful-backup`.

Before compression, the plain dump passes through a fail-closed retention sanitizer. A first pass over one internally consistent `pg_dump` snapshot verifies the exact `youtube_discovery` policy and requires exactly one supported `CREATE UNLOGGED TABLE ingest.youtube_discoveries` header in that same spool; a second pass removes every data row from that table while preserving its schema. A logged, temporary, missing, duplicate, or unsupported target-table definition fails before any output byte is produced. Generic `ingest.source_items` rows are not selected or removed. The raw snapshot is spooled only to an unlinked mode-`0600` temporary file. `psql` independently validates the live policy, table and `UNLOGGED` state without putting the database URL in arguments; the dump mapping remains authoritative and any mismatch, missing/ambiguous policy or table, unsupported COPY/INSERT shape, command failure or malformed row aborts the backup without advancing the success marker. Consequently, managed logical backups cannot extend this YouTube API cache beyond its 28-day database lifecycle.
