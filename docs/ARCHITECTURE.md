...
The deployment artifacts define `collector`, `ai-worker`, `aggregator`, `scheduler`, and `watchdog` as roles of one worker image with role-specific commands and least-privilege environment variables. The live composition is deliberately single-process (`WORKER_MAX_CONCURRENCY=1`). Cleanup and the twice-daily (02:00 and 14:00 UTC) TCGdex English sets-catalog sync are fixed jobs. Durable schedule slots make the two windows independent and deduplicated, including bounded catch-up after a missed window. When the explicit YouTube collection flag is enabled, the scheduler enqueues one fenced job for each of five versioned global discovery queries on an exact six-hour schedule; any cadence drift fails startup, and the scheduler does not receive the API key. Only the collector receives the dedicated key and may claim those jobs. General URL and browser collection, `ai-worker`, and `aggregator` still fail closed. TCGdex rows are catalog-only. YouTube rows are tier-D/activity-only metadata. The bounded social-volume projector may turn an explicit pack number from a YouTube title or Bluesky excerpt into a `reported_volume` row containing only a public URL, count, hash and precision. It creates no cards, openings, hits, denominators, aggregates, or pull-rate evidence; title claims never enter page-check work. `auth-browser` has its own Chromium/noVNC image and sensitive persistent profile volume.
...
The worker login inherits `service_role`, but the forward migration leaves that role without direct application-table row mutation and denies request-gate reads. The gate's sole direct exception is PostgreSQL 17 `MAINTAIN` for the logical backup schema lock; it does not expose rows, but it is another reason to separate dedicated worker and backup credentials before enablement. Enqueue, generic no-effect completion, budget pause, health heartbeat, and every persistent collector effect cross narrow `SECURITY DEFINER` RPCs; current typed jobs cannot use generic completion to bypass their finalizers.

Admin public RPCs are a PostgREST-only boundary: their non-public implementations live in `ingest`, while the stable public wrappers require both the immutable `authenticator` login session and a `service_role` JWT claim. A direct database worker cannot satisfy that session check by inheriting or setting the service role.

Every persistent collector has its own transactional database boundary. Cleanup uses `finalize_cleanup_job`. TCGdex uses a fenced policy/request-gate preflight and `finalize_tcgdex_sets_job`. YouTube discovery uses `begin_youtube_discovery_job` before one bounded `search.list` call and `finalize_youtube_discovery_job` afterward. The finalizer rechecks the exact lease, generation, policy, request gate, result version, identities, and size bounds before atomically upserting the dedicated `UNLOGGED`, forced-RLS `ingest.youtube_discoveries` cache and completing the job. The scheduler's separate `sync-social-volume` command reads only bounded, expiring YouTube/Bluesky fields and imports a quantity-only manifest; `process-global-volume` records title claims without refetching them. Those caches have no path into generic source items, extraction, openings, analytics, Admin, or statistical relations. A stale lease, disabled kill switch, malformed response, or identity collision produces no partial persistence. Future persistent handlers still require their own typed finalizer and external idempotency. PostgreSQL is already required and is adequate for expected low concurrency. Omitting Redis removes another credentialed, patched, monitored, backed-up stateful service. Add a broker only after measured latency/throughput or lock pressure proves this design insufficient. Repository contracts are not evidence that a migration or worker revision has been deployed.
...
3. Metadata/excerpts are normalized and deduplicated according to each source's exact contract. YouTube retains only the video identity, canonical URL, title, publication timestamp, and lifecycle fields; video, audio, captions, thumbnails, descriptions, query/rank associations, channel data, and derived classifications are not retained. The quantity projector reads the title or bounded Bluesky excerpt transiently and retains only a count, public URL, report-group hash and precision; it never stores account identity or follower metrics.
...
7. Public v1 aggregation publishes only the reviewed Australia/English statistical slice. The separate `reported_volume` projection may publish bounded source-reported pack totals, while social activity metadata and source text stay private.
...
- The logical database backup stream strips all data rows from the dedicated
  ...
  dump fails without advancing the success marker. It excludes request-gate
...
Missing live configuration yields unavailable/failure, never silent demo substitution. A collector error leaves the job retryable/dead-lettered; it does not fabricate an opening. If curl cannot be confirmed reaped, the worker crosses a fatal boundary instead of finalizing the job, leaving the persistent request gate fenced until its database lease expires. Shutdown control flow likewise propagates without normal failure finalization. AI budget exhaustion pauses AI work. Health failure prevents the deploy success marker. Database migrations and external deployments remain explicit operator actions.
