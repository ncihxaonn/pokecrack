...
class MastodonRequestLimiter:
    """Add fixed in-process pacing; the database gate is the global limiter.

    This lock only protects requests made by one collector process.  Workers
    must never use it as the cross-worker authority: the fenced begin RPC
    reserves the shared Mastodon request gate and policy window in Postgres.
    """
...
class MastodonPublicHashtagCollector:
    ...
    def __init__(
        ...
        *,
        ...
        # A limiter belongs to this collector instance only.  Cross-worker
        # pacing is owned by the fenced Postgres request gate, not a process
        # singleton in this module.
        self.limiter = limiter if limiter is not None else MastodonRequestLimiter()
