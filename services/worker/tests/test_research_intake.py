...
def manifest() -> dict:
    return {"schema_version": SCHEMA_VERSION, "snapshot_sha256": "a" * 64,
            "references": [{"url": "https://example.com/study",
                            "report_group_sha256": "b" * 64, "conflicting": False}]}
...
def outcome(**changes: object) -> dict:
    return {"status": "accepted", "snapshot_sha256": "a" * 64, "references_received": 1,
            "references_inserted": 1, "reference_count": 1, "conflicting_count": 0,
            "family_queued": 0, **changes}
...
def test_intake_accepts_only_private_references_and_uses_one_parameterized_query() -> None:
    ...
    executor.query.assert_called_once_with(IMPORT_SQL, {"manifest": json.dumps(manifest(), sort_keys=True)})
    assert "http" not in IMPORT_SQL and "public." not in IMPORT_SQL


@pytest.mark.parametrize("value", [
    None, [], {}, {**manifest(), "packs": 30}, {**manifest(), "schema_version": None},
    {**manifest(), "schema_version": "self-approved"},
    {**manifest(), "snapshot_sha256": "A" * 64}, {**manifest(), "references": None},
...
@pytest.mark.parametrize("update", [
    {"packs": 1000}, {"country": "JP"}, {"approved": True}, {"html": "private body"},
    {"conflicting": 0}, {"conflicting": None}, {"report_group_sha256": None},
    {"url": "http://example.com/study"}, {"url": "https://127.0.0.1/study"},
    {"url": "https://user:secret@example.com/study"},
    {"url": "https://example.com/study?secret=value"},
    {"url": "https://example.com/study#fragment"}, {"url": "https://example.local/study"},
    {"url": "https://example.com:443/study"}, {"url": "https://example.com:bad/study"},
    {"url": "https://EXAMPLE.com/study"}, {"url": "https://example.com/study/"},
    {"url": "https://example.com/a\nb"}, {"url": "https://example.com/`x`"},
    {"url": "https://example.com/<x>"}, {"url": "https://example.com/a\\b"},
])
...
@pytest.mark.parametrize("result", [
    None, {**outcome(), "url": "https://example.com/private"},
    outcome(status="approved"), outcome(snapshot_sha256="c" * 64),
    outcome(references_received=2), outcome(references_inserted=2),
    outcome(reference_count=0), outcome(conflicting_count=2),
    outcome(family_queued=2), outcome(references_inserted=True),
    outcome(status="paused"), outcome(reference_count=10001),
])
...
def test_cli_dry_run_has_no_settings_or_database_access(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = CliRunner().invoke(cli.app, ["intake-research", "--dry-run"], input=encoded(manifest()))
    assert result.exit_code == 0, result.output
    assert json.loads(result.stdout) == {"dry_run": True, "mutated": False, "references": 1}
    settings.assert_not_called()


@pytest.mark.parametrize("mode,role", [(DataMode.DEMO, "collector"), (DataMode.LIVE, "scheduler"),
                                      (DataMode.LIVE, None), (DataMode.LIVE, "nostr-collector")])
def test_cli_rejects_non_live_collector(monkeypatch: pytest.MonkeyPatch, mode: DataMode, role: str | None) -> None:
...
def test_cli_redacts_database_failure_and_handles_success(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(cli, "_settings", lambda: SimpleNamespace(data_mode=DataMode.LIVE, worker_role="collector"))
    executor = Mock()
