...
-- Keep the established v2 browser endpoint stable while adding a strictly
-- denominator-safe source-level coverage tuple. The tuple is emitted only
-- from a reviewed source's valid, current rolling-period row; no source with
-- missing or invalid coverage is represented as a zero.
...
comment on function public.get_public_study_coverage_v2() is
  'Registry-driven public projection of all reviewed denominator-safe opening studies. Per-source coverage appears only for a valid current-period row; it never returns evidence, numerator, policy, job, gate, or inference fields.';
