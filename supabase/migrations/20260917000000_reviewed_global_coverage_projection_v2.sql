...
-- Public coverage used to be split between the first two reviewed studies in
-- the v3 dashboard snapshot and a second, hard-coded three-study supplement.
-- That made every additional approved study a coordinated frontend release
-- and could add independent-source counts across separate projections.  This
-- owner-side projection reads the immutable reviewed-contract registry once,
-- validates either private evidence ledger, and emits only denominator-safe
-- facts.  It never emits a numerator, evidence text/hash, source policy ID,
-- job, gate, or statistical inference.
...
comment on function public.get_public_study_coverage_v2() is
  'Registry-driven public projection of all reviewed denominator-safe opening studies. It deduplicates independent domains server-side and never returns evidence, numerator, policy, job, gate, or inference fields.';
