...
select is(
  ...
  array['bluesky_jetstream', 'disabled', 'manual_import', 'official_api', 'opencli_authenticated', 'scrapling_dynamic', 'scrapling_http']::text[],
  'source policies use exactly the seven collector registry values'
);
select is(
  (select array_agg(matched.value[1] order by matched.value[1])
   from pg_constraint c
