...
select ok(
  ...
  'reviewer role is an isolated NOLOGIN, NOINHERIT capability with no role settings'
);
select ok(
  not exists (
    select 1
...
select ok(
  not exists (
    select 1
    from pg_auth_members as memberships
    join pg_roles as login on login.oid = memberships.member
    where memberships.roleid = (
      select reviewer.oid
      from pg_roles as reviewer
      where reviewer.rolname = 'pokecrack_authorized_opening_reviewer'
  ...
  'reviewer capability members are dedicated non-inherited logins'
);
do $reviewer_membership_fixture$
begin
  create role pokecrack_authorized_opening_reviewer_test_login
  ...
  grant pokecrack_authorized_opening_reviewer
    to pokecrack_authorized_opening_reviewer_test_login
...
select ok(
  (select count(*) = 1
   ...
   where memberships.roleid = (
     ...
     and login.rolname = 'pokecrack_authorized_opening_reviewer_test_login'
  ...
  'the intended dedicated login membership points into the reviewer capability'
);
select ok(
  ...
  not exists (
    ...
    where namespaces.nspname in ('catalog', 'ingest', 'analytics', 'public')
      and has_schema_privilege(
        'pokecrack_authorized_opening_reviewer', namespaces.oid, 'create'
      )
  ),
  'reviewer role cannot create objects in any application schema'
);
select ok(
...
select ok(
  not exists (
    ...
    where namespaces.nspname in ('catalog', 'ingest', 'analytics', 'public')
      ...
      and (
        has_table_privilege('pokecrack_authorized_opening_reviewer', relations.oid, 'select')
        or has_table_privilege('pokecrack_authorized_opening_reviewer', relations.oid, 'insert')
        or has_table_privilege('pokecrack_authorized_opening_reviewer', relations.oid, 'update')
        or has_table_privilege('pokecrack_authorized_opening_reviewer', relations.oid, 'delete')
        or has_table_privilege('pokecrack_authorized_opening_reviewer', relations.oid, 'truncate')
        or has_table_privilege('pokecrack_authorized_opening_reviewer', relations.oid, 'references')
        or has_table_privilege('pokecrack_authorized_opening_reviewer', relations.oid, 'trigger')
        or has_table_privilege('pokecrack_authorized_opening_reviewer', relations.oid, 'maintain')
  ...
  and not exists (
    ...
    where namespaces.nspname in ('catalog', 'ingest', 'analytics', 'public')
      ...
      and (
        has_sequence_privilege('pokecrack_authorized_opening_reviewer', sequences.oid, 'usage')
        or has_sequence_privilege('pokecrack_authorized_opening_reviewer', sequences.oid, 'select')
        or has_sequence_privilege('pokecrack_authorized_opening_reviewer', sequences.oid, 'update')
      )
  ),
  'reviewer has no direct application table or sequence privilege'
);
select ok(
  not exists (
    select 1
