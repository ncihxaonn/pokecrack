...
MIGRATION = (
    SUPABASE_ROOT / "migrations/20260907000000_authorized_opening_review.sql"
).read_text()
...
class AuthorizedOpeningContractTests(unittest.TestCase):
    ...
    def test_migration_exposes_only_the_approved_rpc_acl(self) -> None:
        ...
        self.assertIn("reviewer_role_is_safe", lowered)
        self.assertIn("not rolsuper", lowered)
        self.assertIn("must be pre-provisioned as nologin", lowered)
        self.assertNotIn("alter role pokecrack_authorized_opening_reviewer", lowered)
    ...
    def test_public_projection_keeps_rates_and_private_fields_withheld(self) -> None:
        ...
        self.assertIn("'\"2.0.0\"'::jsonb", public_body)
