...
select is(
  ...
  array(select generate_series(1, 31)), 'all 31 reviewed contract ordinals exist'
);
select is(
...
select ok(ingest.reviewed_public_study_gates_ready_v1(), '31 policy gates are ready');
select ok(exists (
