...
export default async function AdminBrowserPage() { const access = await requireAdmin(); const snapshot = await getAdminSnapshot(access); return <><AdminHeading title="Browser sessions" description="Opaque profile health only—never browser cookies, tokens or remote-control secrets." /><AdminModeNotice message={snapshot.message} synthetic={snapshot.synthetic} /><AdminBrowserView data={snapshot.status === "ready" ? snapshot.data : null} /></>; }
