...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (result.status === "unavailable") return createPageMetadata("Coverage data unavailable", result.message, "/regions", { index: false });
  const region = result.data.regions.find((candidate) => candidate.slug === slug);
  if (!region) notFound();
  return createPageMetadata(region.name, `Declared country or product-market coverage and aggregate sample summary for ${region.name}.`, `/regions/${region.slug}`);
}
