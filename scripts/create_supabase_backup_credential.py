...
URL nor either credential is printed.  The generated URL uses the IPv4 session
pooler and starts every connection as ``postgres`` so the existing reviewed
backup pipeline retains the same effective-role contract as a dedicated owner
login.
"""
...
class SupabaseManagementAPI:
    ...
    def pooler_config(self) -> object:
        return self._request(method="GET", path="/config/database/pooler")
...
def _primary_pooler_connection(payload: object) -> str:
    if not isinstance(payload, list) or any(not isinstance(item, dict) for item in payload):
        raise TemporaryCredentialError("Supabase pooler response had an ambiguous shape")
    primary = [item for item in payload if item.get("database_type") == "PRIMARY"]
    if len(primary) != 1:
        raise TemporaryCredentialError(
            "Supabase pooler response did not contain exactly one primary"
        )
    snake = primary[0].get("connection_string")
    camel = primary[0].get("connectionString")
    values = [value for value in (snake, camel) if value is not None]
    if (
        not values
        ...
        raise TemporaryCredentialError(
            "Supabase primary pooler connection string was ambiguous"
        )
    return values[0]
...
def build_database_url(
    *, pooler_connection: str, project_ref: str, role: str, password: str
) -> str:
    placeholder = ":[YOUR-PASSWORD]@"
    if pooler_connection.count(placeholder) > 1:
        raise TemporaryCredentialError("Supabase pooler connection string was invalid")
    parseable_connection = pooler_connection.replace(placeholder, ":@")
    try:
        parsed = urllib.parse.urlsplit(parseable_connection)
    except ValueError as error:
        raise TemporaryCredentialError(
            "Supabase pooler connection string was invalid"
        ) from error
    ...
    try:
        ...
        raise TemporaryCredentialError(
            "Supabase pooler connection string had an invalid port"
        ) from error
    if (
        ...
        raise TemporaryCredentialError(
            "Supabase primary pooler connection string was outside the reviewed contract"
        )
    if parsed.query:
        query = urllib.parse.parse_qsl(
            parsed.query, keep_blank_values=True, strict_parsing=True
        )
        if query and query != [("sslmode", "require")]:
            raise TemporaryCredentialError(
                "Supabase primary pooler connection string had unexpected options"
            )

    username = urllib.parse.quote(f"{role}.{project_ref}", safe="")
    encoded_password = urllib.parse.quote(password, safe="")
    options = urllib.parse.quote("-c role=postgres", safe="")
    return (
        f"postgresql://{username}:{encoded_password}@{parsed.hostname}:5432/postgres"
        f"?sslmode=verify-full&sslrootcert=system&connect_timeout=15"
...
def create(
    ...
    *,
    ...
    pooler = _primary_pooler_connection(api.pooler_config())
    if _active_cli_login_roles(api):
        raise TemporaryCredentialError(
