#!/usr/bin/env bash
# Queue a bounded quantity-only public-report manifest on the reviewed worker.
# The manifest is validated again by the worker and the database RPC.
set -Eeuo pipefail
umask 077
...
exec timeout --foreground --kill-after=5 90 docker exec -i "$container_id" \
  pokecrack-worker intake-global-volume
